...
export default function Sidebar() {
  ...
  const isActive = (id: string) => {
    ...
    if (id === 'assessments') return pathname.startsWith('/assessments');
    if (id === 'question-bank') return pathname === '/question-bank';
    if (id === 'candidates') return pathname === '/candidates';
  ...
  return (
    <aside className="w-64 bg-white border-r border-gray-200 flex flex-col">
        ...
        {navItems.map((item) => (
          <Link
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors mb-1 ${
              isActive(item.id) ? 'bg-indigo-50 text-indigo-600' : 'text-gray-700 hover:bg-gray-50'
            }`}
          >
      ...
      <div className="p-4 m-4 bg-indigo-50 rounded-lg border border-indigo-100">
