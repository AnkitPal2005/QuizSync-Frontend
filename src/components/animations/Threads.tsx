...
function Threads({ color = [1, 1, 1], amplitude = 1, distance = 0, enableMouseInteraction = false, className }: ThreadsProps) {
  ...
  useEffect(() => {
    ...
    const update = (time: number) => {
      if (enableMouseInteraction) {
        ...
        program.uniforms.uMouse.value.set(currentMouse[0], currentMouse[1], 0);
      } else {
        program.uniforms.uMouse.value.set(0.5, 0.5, 0);
      }
